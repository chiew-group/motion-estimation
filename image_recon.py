...
if __name__ == '__main__':
    ...
    device = cp if  args.gpu else np

    print('-' * 50)
    print("Experiment Starting")
    print(f"Gpu usage: {args.gpu}")
    print(f"Downsampled by a factor of {args.ds_factor}")
    ...
    kgrid, rkgrid = compute_transform_grids_voxel(img_shape, voxel_size, xp=device)
    ...
    # Recon
    recon = estimate_image_cg(ksp, mps, shot_mask, t0, kgrid, rkgrid, max_iter=args.max_iter)
    recon = cp.asnumpy(cp.fft.fftshift(recon))
    
    np.save(os.path.join(args.out_dir, f"full_recon_{num_shots}shots.npy"), recon)